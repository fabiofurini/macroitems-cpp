#!/usr/bin/env python3
"""
Benchmark script for large instances (n = 60000..100000).

Runs only the fast heap-based algorithms (forest, inforest, outforest).
forest_noheap is intentionally excluded: its O(n^2) cost makes it
impractical at these sizes.

Appends results to results/runs.csv (same format as run_benchmark.py),
then rebuilds the four timing CSVs in report_performance/.

Run from the project root:
    python3 report_performance/run_benchmark_large.py
"""
...
# Only the new large sizes
SIZES_LARGE = [60000, 70000, 80000, 90000, 100000]
...
# Fast algorithms only – forest_noheap excluded
RUNS_PLAN = [
    # (topo,       algo,        sizes)
    ("forest",    "forest",    SIZES_LARGE),   # HFMA on gen-forest
    ("inforest",  "inforest",  SIZES_LARGE),   # HIMA
    ("inforest",  "forest",    SIZES_LARGE),   # HFMA on in-trees (for speedup)
    ("outforest", "outforest", SIZES_LARGE),   # HOMA
    ("outforest", "forest",    SIZES_LARGE),   # HFMA on out-trees (for speedup)
]

total = sum(len(sz) * len(CLASSES) * len(DENSITIES) * len(SEEDS)
            for _, _, sz in RUNS_PLAN)
done = errors = 0
...
for topo, algo, sizes in RUNS_PLAN:
    ...
    for n in sizes:
        for kp in CLASSES:
            for d in DENSITIES:
                for s in SEEDS:
                    inst = (f"instances/{topo}/{topo}_{kp}_{d}"
                            f"_n{n:07d}_s{s:02d}.txt")
...
# ── Rebuild timing CSVs from the full results/runs.csv ───────────────────────
...
# All sizes (small + medium + large) for complete CSVs
SIZES_SMALL  = list(range(10, 110, 10))
SIZES_MEDIUM = SIZES_SMALL + list(range(200, 1100, 100))
SIZES_ALL    = SIZES_MEDIUM + [10000, 20000, 30000, 40000, 50000,
